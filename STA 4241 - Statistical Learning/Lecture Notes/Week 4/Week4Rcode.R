...
mean((testPredQDA - (as.numeric(SmarketTest$Direction) - 1))^2)
...
for (ni in 1 : nSim) {
  ...
  errorMat[ni,3] = mean((testPredQDA - (as.numeric(SmarketTest$Direction) - 1))^2)
  
  ## Coin flip estimator
  errorMat[ni,4] = mean((rbinom(500, 1, 0.5) - (as.numeric(SmarketTest$Direction) - 1))^2)
