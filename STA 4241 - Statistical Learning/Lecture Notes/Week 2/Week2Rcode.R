...
linMod = lm(y ~ x)
predictions1 = linMod$coefficients[1] + linMod$coefficients[2]*s

## q = 3 degree of freedom polynomials
x3 = cbind(x, x^2, x^3)
s3 = cbind(s, s^2, s^3)
linMod3 = lm(y ~ x3)
predictions3 = cbind(rep(1, length(s)), s3) %*% linMod3$coefficients

## q = 5 degree of freedom polynomials
x5 = cbind(x, x^2, x^3, x^4, x^5)
s5 = cbind(s, s^2, s^3, s^4, s^5)
linMod5 = lm(y ~ x5)
predictions5 = cbind(rep(1, length(s)), s5) %*% linMod5$coefficients
