...
fitRidge = glmnet(x = mice$x[,11:20], y = mice$y[,1], alpha = 0)
...
for (m in 1 : M) {
  ...
  ## Generate new data
  xnew = mvtnorm::rmvnorm(60, sigma=sigma)
  ynew = xnew %*% beta + rnorm(60)
...
axis(1, at=seq(0.5, 11.5, by=1), labels=paste("PC", 1:12), las=2)
