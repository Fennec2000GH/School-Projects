...
for (ii in 1 : nSim) {
  ...
  samp = sample(1:nrow(Auto), 192, replace=FALSE)
  trainAuto = Auto[samp,]
  testAuto = Auto[-samp,]
...
## For each of the 20 data splits, which degree of polynomial is chosen
...
## randomly order the indices of the data
randOrder = sample(1:nrow(Smarket), nrow(Smarket), replace=FALSE)

## kGroups will be a list of length 10 that identifies which indices we want to keep
kGroups = list()
for (k in 1 : K) {
  kGroups[[k]] = ((k-1)*nrow(Smarket)/10 + 1) : (k*nrow(Smarket)/10)
}
