...
## generate testing data sets for each of the four scenarios
...
data_test_var = data.frame(y = c(rep(1, n_test/2), rep(0,n_test/2)))
data_test_var$x = rep(NA, n_test)

data_test_var$x[1:(n_test/2)] = rnorm(n_test/2, mean=1, sd=1)
data_test_var$x[(n_test/2 + 1):n_test] = rnorm(n_test/2, sd=10)
...
## store results
nSim = 100
test_error_lda = matrix(NA, nSim, 4)
test_error_log = matrix(NA, nSim, 4)
...
for (ni in 1 : nSim) {
  ...
  ## Now generate data where the constant variance assumption is incorrect
  data_var = data.frame(y = c(rep(1, n/2), rep(0,n/2)))
  data_var$x = rep(NA, n)
  
  data_var$x[1:(n/2)] = rnorm(n/2, mean=1, sd=1)
  data_var$x[(n/2 + 1):n] = rnorm(n/2, sd=10)
  
  ## fit logistic model and store error rate
  modLOG = glm(y ~ x, family=binomial, data=data_var)
  pred_testLOG = 1*(predict(modLOG, data_test_var, 
                            type="response") > 0.5)
  test_error_log[ni,4] = mean(pred_testLOG != data_test_var$y)
  
  ## Fit LDA model and store error rate
  modLDA = lda(y ~ x, data=data_var)
  pred_testLDA = as.numeric(predict(modLDA, newdata=
                                      data_test_var)$class) - 1
  test_error_lda[ni,4] = mean(pred_testLDA != data_test_var$y)
}
...
## remove the 2 by 2 plotting window
dev.off()
